...
class _TorchDistributedEnvironment:
    ...
    def fix_on_juwels(self):
        ...
        if "jureca" in self.master_addr:
            _split = self.master_addr.split(".")
            if len(_split) == 2 and _split[1] == "jureca" and _split[0][-1] != "i":
                self.master_addr = _split[0] + "i" + "." + _split[1]
...
def enable(
    *,
    ...
    if num_processes > 0:
        threads_per_process = math.floor(total_cores / num_processes)
    else:
        threads_per_process = 1  # Default to 1 if no division is possible
