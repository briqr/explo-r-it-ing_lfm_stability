...
class FrechetInceptionDistance(Metric):
    ...
    def update(self, imgs: Tensor, real: bool) -> None:
        ...
        if len(imgs.shape) == 5:
            ...
            raise ValueError(
                f"Input images should have shape (B, C, H, W) or (T, B, C, H, W) get shape: {imgs.shape}"
            )
    ...
    def compute(self) -> Tensor:
        ...
        real_features_sum = self.real_features_sum.clone().double()
        fake_features_sum = self.fake_features_sum.clone().double()
        real_features_cov_sum = self.real_features_cov_sum.clone().double()
        fake_features_cov_sum = self.fake_features_cov_sum.clone().double()
