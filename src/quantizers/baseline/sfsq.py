...
class SFSQ(Module):
    def __init__(
        ...
        **kwargs,
        ...
        implicit_codebook = self.indice_to_code(
    ...
    def latent_to_indice(self, zhat: Tensor) -> Tensor:
        """Converts a `code` to an index in the codebook."""
        assert zhat.shape[-1] == self.codebook_dim
        zhat = self._scale(zhat)
        return (zhat * self._basis).sum(dim=-1).to(int32)

    def indice_to_code(self, indices: Tensor, project_out=True) -> Tensor:
        """Inverse of `latent_to_indice`."""
    ...
    def forward(self, z: Tensor) -> Tensor:
        ...
        indices = self.latent_to_indice(codes)
