...
class Transport:
    ...
    def training_losses(self, model, x1, x0=None, model_kwargs=None, t=None):
        ...
        if t is None:        
            t, x0, x1 = self.sample(x1)
            t, xt, ut = self.path_sampler.plan(t, x0, x1) #todo rania
        else: #for the training loss on [0,t_0)
            B = x1.size(0)
            if not th.is_tensor(t):
                t = th.full((B,), float(t), device=x1.device, dtype=x1.dtype)
            else:
                t = t.to(device=x1.device, dtype=x1.dtype)
                if t.ndim == 0:
                    t = t.expand(B)
            if x0 is None: # x0 is passed when scoring a sample based on grad
                x0 = th.randn_like(x1)      
            t, xt, ut = self.path_sampler.plan(t, x0, x1)
    ...
    @torch.no_grad()
    def invert_to_t_pf_ode(self, x_data, model, t_star, steps=40, method="euler", model_kwargs=None):
        ...
        if not torch.is_tensor(t_star):
            t_star = torch.full((B,), float(t_star), device=x.device, dtype=x.dtype)
        else:
            t_star = t_star.to(device=x.device, dtype=x.dtype)
            if t_star.ndim == 0: t_star = t_star.expand(B)

        base = torch.linspace(1.0, 0.0, steps + 1, device=x.device, dtype=x.dtype)[None, :]   # [1,S+1]
        ts   = t_star[:, None] + (1.0 - t_star[:, None]) * base                               # [B,S+1]

        for i in range(steps):
            t0 = ts[:, i]     
            t1 = ts[:, i + 1]  
            h  = (t1 - t0).view(B, *([1] * (x.ndim - 1)))  # negative step
            v0 = drift(x, t0, coarse_model, **model_kwargs)
            if method.lower() == "euler":
                x = x + h * v0
            else:  # Heun
                x1_pred = x + h * v0
                v1 = drift(x1_pred, t1, coarse_model, **model_kwargs)
                x  = x + 0.5 * h * (v0 + v1)
        return x  
